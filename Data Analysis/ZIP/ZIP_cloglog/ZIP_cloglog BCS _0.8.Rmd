---
title: "Bayesian Credible Subgroups( ZIP clog model) log(time) as a offset"
...
```{r,echo=FALSE}
PoisJag = function(Model,data, nIter){
 ...
 offset.variable=covariate$month
  ...
  ###JAGS for zipmodel1
  jagsmodel1 <- jags.model(textConnection(zipModelkm_clog_off),data=datalist1,quiet=TRUE)
...
```{r,echo=FALSE}
##########################################################
nIter=10000;zeta.diff = 0;cred.level=0.8
###########################################################

JagResults=PoisJag(PoisJag,data,nIter)
...
CalRegression = function(design, coef.zero, coef.pois){
 ...
 X.design = cbind(1,trt=design$treat,initnr=design$initnr,
  ...
  ##empty matrices
  e.zero = matrix(0, nIter, nsubj)#(theta) = probability rv excess zero
  e.pois = matrix(0, nIter, nsubj)#(mu) mean of the poisson.
  
  for(isubj in 1:nsubj){
    e.zero[,isubj] =
     (  as.vector(exp(-exp(Z.design[isubj,]%*%t(coef.zero)))))
...
#safety.ratio = matrix(0, nIter, nsubj)
safety.diff  = matrix(0, nIter, nsubj)


#for(isubj in 1:nsubj){
#  safety.ratio[,isubj] = ((1-e.trt$e.zero[,isubj])*(e.trt$e.pois[,isubj]))/
 #   ((1-e.ctrl$e.zero[,isubj])*(e.ctrl$e.pois[,isubj]))
#}

#safety.diff is used to calculate the Estimated Personalized Treatment Effect.
for(isubj in 1:nsubj){
  safety.diff[,isubj] = ((1-e.trt$e.zero[,isubj])*(e.trt$e.pois[,isubj]))-
    ((1-e.ctrl$e.zero[,isubj])*(e.ctrl$e.pois[,isubj]))
}
